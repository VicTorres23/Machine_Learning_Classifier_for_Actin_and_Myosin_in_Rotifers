...
training_dataset = pd.read_csv("/Users/victor_torres/PycharmProjects/Actin_Myosin/Fixed_Myosin2_Train_Dataset.csv")

class_counts = training_dataset['Classification'].value_counts()
small_classes = class_counts[class_counts < 2].index.tolist()

if small_classes:
    print(f"⚠️ Removing classes with less than 2 samples: {small_classes}")
    training_dataset = training_dataset[~training_dataset['Classification'].isin(small_classes)]
else:
    print("✅ No classes with less than 2 samples.")
...
test_data = pd.read_csv("/Users/victor_torres/PycharmProjects/Actin_Myosin/Phylo_Tree_Test.csv")
titles = test_data["Title"]
X_unlabeled = test_data.drop(columns=["Title"])

missing_cols = [col for col in X.columns if col not in X_unlabeled.columns]
extra_cols = [col for col in X_unlabeled.columns if col not in X.columns]

for col in missing_cols:
    X_unlabeled[col] = 0

X_unlabeled = X_unlabeled.drop(columns=extra_cols)
...
results_df.to_csv("New_Myosin_Classification_Prediction_Phylo_Tree.csv", index=False)
